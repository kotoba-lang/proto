(ns proto.core
  "Protocol Buffers schema as data — 'hiccup for wire schemas'. A .proto file is a declarative message/
   ...
   you fork and diff like a scene. A schema/IDL sibling in the kotoba.* family (next to kotoba.wgsl's WIT
   lineage). `.cljc`.

   proto3 is a tree of declarations (not infix), so no kotoba.expr. Items:
